...
# ========== Game 1: Scenario Simulator ==========
    ...
    if st.session_state.step == 1:
        ...
        if st.session_state.savings >= 40:
            st.balloons()
            st.write("🎉 Amazing! You're a smart saver.")
        elif st.session_state.savings >= 20:
            st.write("👍 Nice job. You're learning to make better choices!")
        else:
            st.write("💡 It's okay to have fun, but remember to save too!")
...
# ========== Game 2: Budget Slider Game ==========
    ...
    if total > 1000:
        st.error("🚫 You're over budget! Adjust your sliders.")
    elif total < 1000:
        st.warning("⚠️ You haven't used your full budget. Consider saving more!")
    else:
        score = 0
...
# ========== Game 3: Needs vs. Wants ==========
    ...
    st.header("🧠 Needs vs. Wants Sorter")
    ...
    if st.button("Check Answers"):
        ...
        if correct == len(items):
            st.balloons()
            st.success("🎉 Perfect! You really know your priorities.")
        elif correct >= 3:
            st.success("👍 Great job! You're on the right track.")
        else:
            st.info("💡 Keep practicing! Needs come before wants when budgeting.")
